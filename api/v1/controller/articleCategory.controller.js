...
module.exports.articleCategory = async (req, res) => {
    try {
        ...
        const articleData = await Promise.all(articleCategories.map(async (item) => {
                try {
...
module.exports.detail = async (req, res)=>{
    ...
    try {
        const id = req.params.id
        const articleCategory = await ArticleCategory.findOne({_id: id}, {deleted: false});
        res.json({
...
module.exports.changeStatus = async (req, res)=>{
    try {
        ...
        await ArticleCategory.updateOne({_id: id}, {status: status, $push: {updatedBy: updateBy}});
        res.json({
            code: 200,

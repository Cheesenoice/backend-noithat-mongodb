...
module.exports.article = async (req, res) => {
    try {
        ...
        const pagination = paginationHelper(initPagination, req.query, countArticle);
        ...
        const articleData = await Promise.all(articles.map(async (item) => {
            try {
                // Lấy thông tin người tạo
                let accountFullName = "Unknown";
...
module.exports.create = async (req, res) => {
    try {
        ...
        if (req.body.position === "" || req.body.position === undefined || req.body.position === null) {
...
module.exports.changeStatus = async (req, res)=>{
    try {
        ...
        await Article.updateOne({_id: id}, {status: status,  $push: {updatedBy: updatedBy}});
        res.json({
            code: 200,
